...
setwd("~/Desktop/covid_hirief/data/14500_STD_covid_serum_phospho_normalized_20230206_13.41/")

dfna2 = read_delim("ptm_peptides_not_adjusted.txt")
dfadj2 = read_delim("ptm_peptides_total_proteome_adjusted.txt")
dfs = read_excel("../schweizer_2022_medrxiv/media-3.xlsx", sheet=2)
dfpep2 = read_delim("peptides_table.txt")
dfpep = dfpep2[grep("+79.966", dfpep2$`Peptide sequence`),]

# Oxidation of methionine
length(grep("15.995", dfpep2$`Peptide sequence`))
# Phosphorylation of serine, tyrosine, or threonine, and histidine (less common)
length(grep("79.966", dfpep2$`Peptide sequence`))
# Carbamidomethylation - iodoacetamide, alkylation, to cap the SH covalent bonds of cysteine residues in methionine, after reduction with DTT
length(grep("57.021", dfpep2$`Peptide sequence`))
...
dfadj = as.data.frame(dfadj2[,sel3])
colnames(dfadj) = labids$sample_id
rownames(dfadj) = dfna2$gene_name

phlab = read_excel("../GMJKMP1-60_phospho_cell_run/GMJKMP_1-60_peptideC-labelling.xlsx", sheet=3)
cellp2 = read_delim("../14511_STD_phospho-Covid19-cell_lines_normtototal_20230216_12.46/ptm_peptides_not_adjusted.txt")
cellp4 = read_delim("../14511_STD_phospho-Covid19-cell_lines_normtototal_20230216_12.46/ptm_peptides_total_proteome_adjusted.txt")
...
head(cellp4)
sel = grep("tmt16plex_[0-9]+C$|tmt16plex_[0-9]+N$|_tmt16plex_126$", colnames(cellp4))
colnames(cellp4)[sel]
cellp3 = as.data.frame(cellp4[,sel])
cellp3 = apply(cellp3, 2, as.numeric)
hist(cellp3, breaks=100)
boxplot(cellp3)

cellp4$gene_name = make.unique(cellp4$`Gene name(s)`)
rownames(cellp3) = cellp4$gene_name
colnames(cellp3) = phlab$type
...
write.csv(mf2, "phosphopeptides_located_PSPlus.csv", row.names = F)
...
##### T test adjusted levels #####
cov = grep("COVID", colnames(dfadj))
hc = grep("HC", colnames(dfadj))

sel1 = which(dfadj2$`set1_PTM FLR`>0.05)
sel2 = which(dfadj2$`set2_PTM FLR`>0.05)
rem = union(sel1, sel2)

dfadj_rem = dfadj[-rem,]
dfadjll = dfadj_rem

miss = apply(dfadjll, 1, function(x) {length(which(is.na(x)))/length(x)*100})
table(miss<51)
sel = which(miss<51)

dfadjll = dfadjll[sel,]

res = apply(dfadjll, 1, function(x) { t.test(x[cov], x[hc]) })
res2 = lapply(res, unlist)
results_adj = as.data.frame(do.call(rbind, res2))
results_adj$log2fc = apply(dfadjll, 1, function(x) { mean(x[cov], na.rm=T)-mean(x[hc], na.rm=T) })
results_adj$adj.p = p.adjust(results_adj$p.value, method="fdr")
results_adj$gene_name = rownames(dfadjll)

results_adj[,c(1:9,13,14)] = apply(results_adj[,c(1:9,13,14)], 2, as.numeric)
results_adj = cbind(dfna2[match(results_adj$gene_name, dfna2$gene_name), c(1:14)], results_adj)
results_adj$PSPlus_location = mf2$PSPlus_location[match(results_adj$`Peptide sequence`, mf2$`Peptide sequence`)]

res_sign_adj = as.data.frame(results_adj[which(results_adj$p.value<0.05),])
max(res_sign_adj$adj.p)
sel = intersect(res_sign$gene_name, res_sign_adj$gene_name)

up = res_sign$gene_name[which(res_sign$log2fc>0)]
down = res_sign$gene_name[which(res_sign$log2fc<0)]
upa = res_sign_adj$gene_name[which(res_sign_adj$log2fc>0)]
downa = res_sign_adj$gene_name[which(res_sign_adj$log2fc<0)]

intersect(up, upa)
intersect(down, downa)
intersect(up, downa)
intersect(upa, down)

write.csv(results, "serum_phosphopeptides_not_adjusted_all_FLR_filtered.csv")
write.csv(results_adj, "serum_phosphopeptides_adjusted_all_FLR_filtered.csv")
write.csv(res_sign, "serum_phosphopeptides_not_adjusted_sign_5%_fdr_FLR_filtered.csv")
write.csv(res_sign_adj, "serum_phosphopeptides_adjusted_sign_5%_fdr_FLR_filtered.csv")

table(res_sign$`set1_PTM FLR`<0.05)
table(res_sign$`set2_PTM FLR`<0.05)
table(res_sign$set1_PEP<0.05)
table(res_sign$set2_PEP<0.05)
...
ggsave("volcano_serum_phospho_nonadjusted_p<0.05_5percFDR.png", cov, width=8, height=7, units="in", dpi=600)

quartz(type = 'pdf', file = 'volcano_serum_phospho_nonadjusted_p<0.05_5percFDR.pdf', width=8, height=7)
cov
dev.off()

```

```{r adjusted_vs_nonadjusted, include=FALSE}

ov = intersect(res_sign$PSPlus_location, res_sign_adj$PSPlus_location)
res = res_sign[which(res_sign$PSPlus_location %in% ov),]

res$fc_adj = res_sign_adj$log2fc[match(res$PSPlus_location, res_sign_adj$PSPlus_location)]

res$olcat = NA
res$olcat[which(res$log2fc>0 & res$fc_adj>0)] = paste('\U2191', "Non-adjusted,", '\U2191', "Adjusted")
res$olcat[which(res$log2fc>0 & res$fc_adj<0)] = paste('\U2191', "Non-adjusted,", '\U2193', "Adjusted")
res$olcat[which(res$log2fc<0 & res$fc_adj<0)] = paste('\U2193', "Non-adjusted,", '\U2193', "Adjusted")
res$olcat[which(res$log2fc<0 & res$fc_adj>0)] = paste('\U2193', "Non-adjusted,", '\U2191', "Adjusted")
t = table(res$olcat)
a = sum(t[c(1,4)])/sum(t)*100

cv = cor(res$log2fc, res$fc_adj, use="pairwise.complete.obs", method="spearman")
cv

sel = which(res$adj.p<0.05)

p = ggplot(res, aes(log2fc, fc_adj, col=olcat)) +
  geom_point() +
  geom_text_repel(data=res[sel,], label=res[sel,]$PSPlus_location) + 
  theme_classic() +
  ...
  scale_x_continuous("Non-adjusted log2-FC COVID-19:Healthy", breaks=seq(-2,2,0.25)) +
  scale_y_continuous("Adjusted log2-FC COVID-19:Healthy", breaks=seq(-2,2,0.25)) +
...
# quartz(type = 'pdf', file = 'Nonadjusted_vs_adjusted_log2-fc_phosphopeptides_serum.pdf', width=7, height=5)
# p
# dev.off()
# ggsave("Nonadjusted_vs_adjusted_log2-fc_phosphopeptides_serum.jpeg", p, dpi=600, width=8, height=8)
...
for(i in 1:length(sel)) {
  z = sel[i]
  s[[i]] = which(cellp2[,z]>=0.05) # Change cellp2 or 4 depending on non-adjusted/adjusted
}

s2 = unique(unlist(s))
dfsel = cellp[-s2,] # Change cellp or cellp3 depending on non-adjusted/adjusted
dim(dfsel)
phall = na.omit(dfsel)
dim(phall)

mfc2 = read.csv("../data/14511_STD_phospho-Covid19-cell_lines_normtototal_20230216_12.46/phosphopeptides_located_PSPlus_cell_lines.csv")
...
h3 = intersect(unique(ress3h$gene_name), 
          unique(gsub("[.].+", "", results$gene_name)))
d1 = intersect(unique(ress1$gene_name), 
          unique(gsub("[.].+", "", results$gene_name)))
d3 = intersect(unique(ress3$gene_name), 
          unique(gsub("[.].+", "", results$gene_name)))
d7 = intersect(unique(ress7$gene_name), 
          unique(gsub("[.].+", "", results$gene_name)))
h3
d1
d3
...
rest = res7 # change here depending on the day
rest$alteration = "Up in Covid-19"
rest$alteration[which(rest$fc<0)] = "Down in Covid-19"
...
#sel = c(which(rest$shade>quantile(rest$shade, 0.997)), which(rest$p.value==min(rest$p.value)))
ressel = ress7#[sel,]
...
ggsave("volcano_cells_7day_phospho_nonadjusted_5percFDR.png", cov, width=8, height=7, units="in", dpi=600)
 
quartz(type = 'pdf', file = 'volcano_cells_7day_phospho_5percFDR.pdf', width=8, height=7)
cov
dev.off()
...
setwd("../data/14511_STD_phospho-Covid19-cell_lines_normtototal_20230216_12.46/")
...
write.table(mfc, "for_matching_phosphositeplus_cell_lines.txt", row.names = F, quote=F, sep="\t")

plist = list()
for(i in c(1:5)) {
  val = read.delim(paste("PSP-BulkSequenceSearch_", i, ".txt", sep=""), row.names = NULL)
