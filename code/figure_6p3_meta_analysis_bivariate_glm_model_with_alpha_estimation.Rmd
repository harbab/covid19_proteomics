---
title: "Bivariate GLMM model with alpha estimation"
...
# Load data if you don't want to wait for the loops
load("~/Desktop/covid_hirief/meta_analysis_resource_2023.RData")

```

```{r packages and functions, include=FALSE}

### This code was reused from a study by Sehovic et al. Meta-analysis of diagnostic cell-free circulating microRNAs for breast cancer detection. BMC Cancer. 2022 Jun 9;22(1):634. doi: 10.1186/s12885-022-09698-8. PMID: 35681127; PMCID: PMC9178880. 
### The code is implementing a meta-analytical approach in estimating diagnostic accuracy by Doebler P, Holling H. Meta-analysis of Diagnostic Accuracy and ROC Curves with Covariate Adjusted Semiparametric Mixtures. Psychometrika. 2015 Dec;80(4):1084-104. doi: 10.1007/s11336-014-9430-0. Epub 2014 Nov 1. PMID: 25361619.
### Some of the functions are used from the package mada, slightly modified for the purposes of this analysis. The modifications are annotated in the script.
...
##### Extract fit with the option to modify alpha #####
# Extract results from glmer output
fit.glmer.alpha <- function(model, data, alphasens, alphafpr) {
...
```{r sroc_meta_analysis, include=FALSE}
##### ANALYSIS #####
setwd("/Users/haris.babacic/Desktop/covid_hirief/results/processed_data/roc_curves_alpha_c1")
f = list.files()
...
```{r sroc_summary_plots, include=FALSE}
...
#sel = which(is.na(bivglm_res$cat))
#bivglm_res[sel,]
...
```{r alpha_preference_plot, include=FALSE}
...
save.image("~/Desktop/covid_hirief/meta_analysis_resource_2023.RData")

```

```{r lm_vs_glmm_estimation, include=FALSE}
...
```{r SROC_curve_plot_per_protein, include=FALSE}
##### SROC curves #####
...
```{r SROC_curve_top_10, include=FALSE}
